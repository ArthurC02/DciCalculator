...
/// </summary>
public class StrikeSolverService : IStrikeSolver
{
    ...
    public StrikeSolverService(IDciPricingEngine pricingEngine)
    {
    ...
    public IReadOnlyList<(decimal Strike, double Coupon)> GenerateStrikeLadder(
        DciInput input,
        int strikeCount,
        decimal minStrike,
        decimal maxStrike)
    {
        ...
        if (minStrike >= maxStrike)
            throw new ArgumentException("minStrike 必須小於 maxStrike");
    ...
    /// <param name="input">DCI 輸入</param>
    /// <param name="strikeCount">梯形節點數</param>
    /// <param name="belowSpotPercent">低於 Spot 的百分比（如 0.10 表示 10%）</param>
    /// <param name="aboveSpotPercent">高於 Spot 的百分比</param>
    /// <returns>(Strike, Coupon) 列表</returns>
    public IReadOnlyList<(decimal Strike, double Coupon)> GenerateStrikeLadderByPercent(
        DciInput input,
        int strikeCount,
        ...
        double aboveSpotPercent)
    {
        ...
        decimal spotMid = input.SpotQuote.Mid;
        decimal minStrike = spotMid * (1m - (decimal)belowSpotPercent);
        decimal maxStrike = spotMid * (1m + (decimal)aboveSpotPercent);

        return GenerateStrikeLadder(input, strikeCount, minStrike, maxStrike);
    }
    ...
    /// <returns>True 若合理</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool IsStrikeReasonable(decimal strike, decimal spot)
    {
        if (strike <= 0 || spot <= 0)
            return false;
    ...
    /// </summary>
    public double CalculateMaxCoupon(DciInput input)
    {
    ...
    /// </summary>
    public double CalculateMinCoupon(DciInput input)
    {
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private double CalculateCoupon(DciInput input, decimal strike)
    {
        // 建立新的輸入（替換 Strike）
        var modifiedInput = input with { Strike = strike };

        // 報價計算
        var quote = _pricingEngine.Quote(modifiedInput);
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private double CalculateCouponDerivative(DciInput input, decimal strike)
    {
